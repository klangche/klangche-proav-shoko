...
class ProAVShokoGUI:
    ...
    def _on_stop_clicked(self):
        ...
        self.status_label.configure(text="Stopped", fg=self.colors['error'])
    ...
    def _start_analysis(self):
        ...
        self.status_label.configure(text="Analyzing...", fg=self.colors['warning'])
    ...
    def _update_tree_display(self, usb_tree, hops_data, stability, displays):
        ...
        # Determine which port belongs to which category for verdict grouping
        external_verdicts = []
        internal_verdicts = []
        
        # Get verdict platform/platform info
        for v in stability.get('verdicts', []):
            name_lower = v.get('name', '').lower()
            if 'window' in name_lower or 'mac' in name_lower or 'linux' in name_lower or 'android' in name_lower or 'iphone' in name_lower or 'ipad' in name_lower or 'samsung' in name_lower:
                if 'mac' in name_lower and 'apple' in name_lower:
                    internal_verdicts.append(v)
                elif 'mac' in name_lower:
                    external_verdicts.append(v)
                elif 'iphone' in name_lower or 'ipad' in name_lower or 'samsung' in name_lower or 'android' in name_lower:
                    external_verdicts.append(v)
                else:
                    external_verdicts.append(v)
        ...
        if orig_children:
            ...
            # EXTERNAL section
            external_children = [c for c in orig_children if not c.get('is_internal', False) and not c.get('is_display', False)]
            if external_children:
                for idx, child in enumerate(external_children):
                    # Print port header
