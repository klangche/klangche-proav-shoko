...
trap {
    ...
    Write-Host "ÔòöÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòù" -ForegroundColor Red
    Write-Host "Ôòæ                     ERROR DETECTED                          Ôòæ" -ForegroundColor Red
    Write-Host "ÔòÜÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòØ" -ForegroundColor Red
    Write-Host ""
...
function Load-Configuration {
    <#
    .SYNOPSIS
        Load platform data from usb_data.csv on GitHub
    ...
    Write-Host "Shōko - Loading Platform Data" -ForegroundColor Cyan
    Write-Host "==============================================================================" -ForegroundColor Cyan
    Write-Host ""
    
    while ($retryCount -lt $maxRetries) {
        $retryCount++
        Write-Host "[Attempt $retryCount of $maxRetries] Loading platform data from GitHub..." -ForegroundColor Gray -NoNewline
        Write-Host " 🔄" -ForegroundColor Yellow
        
        try {
            if ($CsvPath -and (Test-Path -LiteralPath $CsvPath -ErrorAction SilentlyContinue)) {
                Write-Verbose "Loading CSV from local path: $CsvPath"
            ...
            $platforms = $csvContent | ConvertFrom-Csv
            
            $referenceModels = @()
            $additionalModels = @()
            $refKeys = @("windows_x86", "windows_arm", "mac_intel", "mac_apple_silicon", "linux_x86", "linux_arm")
            
            foreach ($p in $platforms) {
                $name = if ($p.description) { $p.description } else { $p.system }
                $model = [PSCustomObject]@{
                    name = $name
                    rec = [int]($p.max_tiers) - 1
                    max = [int]($p.max_tiers)
                }
                if ($refKeys -contains $p.platform) {
            ...
            $config = [PSCustomObject]@{
                ...
                messages = @{ noDevices = "No USB devices found" }
                scoring = @{
                    minScore = 0
                    maxScore = 10
                    thresholds = @{ stable = 5; potentiallyUnstable = 3 }
                    penalties = @{
                        rehandshake = 0.5
                        jitter = 0.3
                        crc = 1.0
            ...
            Write-Host "`r[Attempt $retryCount of $maxRetries] Loading platform data... ✓" -ForegroundColor Green
            Write-Host ""
            Write-Host "Platform data loaded successfully!" -ForegroundColor Green
            ...
            Write-Host "`r[Attempt $retryCount of $maxRetries] Loading platform data... ✗" -ForegroundColor Red
            Write-Host "  Error: $($_.Exception.Message)" -ForegroundColor Red
    ...
    Write-Host "PLATFORM DATA LOAD FAILED" -ForegroundColor Red
    Write-Host "==============================================================================" -ForegroundColor Red
    Write-Host ""
    Write-Host "Cannot run without platform data. Check your internet connection." -ForegroundColor Yellow
    Write-Host ""
    Write-Host "Options:" -ForegroundColor White
    Write-Host "  y - Retry loading" -ForegroundColor Cyan
...
# =============================================================================
# USB TREE - DUAL MODE (Handles both hierarchical and flat structures)
# =============================================================================

function Get-UsbTree {
    <#
    .SYNOPSIS
        Enumerate USB devices ÔÇô handles both hierarchical trees and flat VM structures
    #>
    param($Config)
    ...
    # If all devices are at depth 2 (VM case), show them in a flat list under a root
    if ($maxHops -eq 2 -and $depthGroups[2].Count -gt 0 -and (-not $depthGroups.ContainsKey(1) -or $depthGroups[1].Count -eq 0)) {
        $treeOutput += "Ôö£ÔöÇÔöÇ USB Controllers (Virtual) ÔåÉ 1 hops`n"
        
        # Group by type for better organization
        $controllers = $depthGroups[2] | Where-Object { $_.Name -like "*controller*" -or $_.Name -like "*host*" } | Sort-Object Name
        $hubList = $depthGroups[2] | Where-Object { $_.IsHub -and $_.Name -notlike "*controller*" } | Sort-Object Name
        $deviceList = $depthGroups[2] | Where-Object { -not $_.IsHub -and $_.Name -notlike "*controller*" } | Sort-Object Name
        
        if ($controllers.Count -gt 0) {
            foreach ($c in $controllers) {
                $treeOutput += "Ôöé   Ôö£ÔöÇÔöÇ $($c.Name) ÔåÉ 2 hops`n"
            }
        }
        ...
        if ($hubList.Count -gt 0) {
            foreach ($h in $hubList) {
                $hubTag = " [HUB]"
                $treeOutput += "Ôöé   Ôö£ÔöÇÔöÇ $($h.Name)$hubTag ÔåÉ 2 hops`n"
            }
        }
        
        if ($deviceList.Count -gt 0) {
            foreach ($d in $deviceList) {
                $treeOutput += "Ôöé   Ôö£ÔöÇÔöÇ $($d.Name) ÔåÉ 2 hops`n"
            }
        }
        
        $treeOutput += "Ôöé   $($Config.messages.noDevices)`n"
    }
    else {
        ...
        function Write-DeviceNode {
            ...
            if ($level -gt 0) {
                $prefix = "Ôöé   " * ($level - 1)
                $prefix += if ($isLast) { "ÔööÔöÇÔöÇ " } else { "Ôö£ÔöÇÔöÇ " }
            } else {
                $prefix = "Ôö£ÔöÇÔöÇ "
            }
            
            $hubTag = if ($node.IsHub) { " [HUB]" } else { "" }
            $script:treeOutput += "$prefix$($node.Name)$hubTag ÔåÉ $($node.Depth) hops`n"
        ...
        if ($roots.Count -eq 0) {
            $treeOutput += "Ôö£ÔöÇÔöÇ USB Root Hub (Host Controller) [HUB] ÔåÉ 1 hops`n"
            $treeOutput += "Ôöé   $($Config.messages.noDevices)`n"
            $maxHops = 1
        } else {
...
function Show-Report {
    ...
    Write-Host "Sh┼ìko - USB + Display Diagnostic Tool v$($Config.version)" -ForegroundColor (Get-Color $Config.colors.cyan)
    Write-Host "==============================================================================" -ForegroundColor (Get-Color $Config.colors.cyan)
    ...
    Write-Host "DISPLAY TREE" -ForegroundColor (Get-Color $Config.colors.cyan)  # Changed from magenta to cyan
    Write-Host "==============================================================================" -ForegroundColor (Get-Color $Config.colors.cyan)
    Write-Host $Display
...
"@
    ...
    $htmlContent = @"
<html>
<head><title>Sh┼ìko Report $dateStamp</title></head>
<body style='background:#000;color:#0f0;font-family:Consolas;'>
<pre>
Sh┼ìko Report - $dateStamp
...
function Start-AnalyticsSession {
    ...
    # Store the position of the stats section
    $statsTop = 6
    $statsBottom = if ($System.IsAdmin) { 17 } else { 13 }
    
    # Initial stats display
    $Host.UI.RawUI.CursorPosition = New-Object System.Management.Automation.Host.Coordinates 0, $statsTop
...
function Main {
    ...
    .SYNOPSIS
        Main execution flow
    #>
    Write-Verbose "Starting Sh┼ìko main script"
    ...
    Write-Host "`nSh┼ìko finished. Press Enter to close." -ForegroundColor (Get-Color $Config.colors.green)
    Read-Host
}
